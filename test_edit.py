...
def visual_validation(accelerator, pipe, dataloader, current_iter, opt):
    ...
    for idx, val_data in enumerate(tqdm(dataloader)):
        ...
        for img, prompt, indice in zip(output, val_data['prompts'], val_data['indices']):
            import re
            img_name = '{prompt}_{indice}'.format(
                prompt=re.sub(r'<[^>]+> <[^>]+>', opt['name'], prompt),
                indice=indice)
            folder_name = re.sub(r'<[^>]+> <[^>]+>', opt['name'], prompt)

            save_img_path = osp.join(opt['path']['visualization'], dataset_name, f'{current_iter}', folder_name, f'{img_name}.png')
